...
export default class TopicForm extends FormView {
  ...
  setLocals(opts) {
    if (opts.topic) {
      this.action = '/api/topic/' + opts.topic.id;
      this.title = 'admin-topics-form.title.edit';
      opts.topic.body = serializer.toHTML(opts.topic.clauses);
    } else {
    ...
    this.forumAdminUrl = ':forum/admin'.replace(':forum', opts.forum ? `/${opts.forum.name}` : '');
