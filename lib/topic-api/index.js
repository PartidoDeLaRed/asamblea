...
var topicListKeys = [
  ...
  'publishedAt deletedAt votable clauseTruncationText links author authorUrl closeMessage'
].join(' ');
  ...
  api.topic.comments(req.params.id, paging, function (err, comments) {
    ...
    if (!req.query.count) {
      ...
      var keys = [
        "id text createdAt editedAt context reference",
        "author.id author.fullName author.displayName author.avatar",
        "flags upvotes downvotes votes replies.length tendency score"
      ].join(' ');
...
app.post('/topic/:id/comment', restrict, function (req, res) {
  ...
  api.topic.comment(comment, function (err, commentDoc) {
    ...
    var keys = [
      'id text',
      'author.id author.fullName author.displayName author.avatar',
      'upvotes downvotes flags tendency',
...
app.post('/topic/:id/message', staff, function (req, res) {
  log('Request POST /topic/%s/message', req.params.id);

  api.topic.get(req.params.id, function(err, topic) {
    if (err) return _handleError(err, req, res);

    topic.closeMessage = req.body.message;
    topic.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
      log('Message set to topic %s', req.params.id);
      res.json(200);
    })
  })
})
