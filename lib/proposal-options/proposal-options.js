...
import ProposalMessage from '../proposal-message/proposal-message';
...
export default class ProposalOptions extends View {
  ...
  switchOn() {
    if (this.proposal.votable) {
      this.renderChart();
    }
    if (this.proposal.closed) {
      var message = new ProposalMessage(this.proposal);
      message.appendTo(this.find('#message-container'))
    }
  }
